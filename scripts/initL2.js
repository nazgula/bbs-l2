...
async function main() {
  ...
  const key = '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80' // '0xe013c13dcf62b53cbe070b00a6886924f58d944d2b43b4aa4050adde3bf7b8e3' //MY WORK METAMASK KEY
  ...
  console.log(`   L1_ERC20 mint 1000000 `)

   await L1_ERC20.mint (l1Wallet.address, 1000000)
...
//  const L2StandardBridge = new ethers.Contract('0x4200000000000000000000000000000000000010', l2StandardBridgeArtifact.abi, l2Wallet)
  const L2StandardBridge = factory__L2StandardBridge
      .connect(l2Wallet)
      .attach('0x4200000000000000000000000000000000000010')


  console.log(`L2StandardBridge.resolvedAddress: ${await L2StandardBridge.resolvedAddress}`)

  console.log('Instantiate L1 Standard Bridge...')
  let L1StandardBridgeAddress = await L2StandardBridge.l1TokenBridge();
  ...
  console.log(`L1StandardBridge.address: ${L1StandardBridge.address}`)
